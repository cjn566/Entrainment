...
void setup(void)
{
  ...
  attachInterrupt(digitalPinToInterrupt(P1_pulse), pulse0, FALLING);
  attachInterrupt(digitalPinToInterrupt(P2_pulse), pulse1, FALLING);
    // RGB
		...
		if (!lights_ready) {
			...
			for (uint8_t i = 0; i < 2; i++)
			{
				if (go_pulse[i]) {
					...
					{
						leds[j + (91 * i)] = CHSV(hue[i] + i*127, sat, waves[pulse_wave_idx[i]]);
					}
